...
function addContentElement(data,statusImg){
	var descriptionId = "description"+data.id;
	var rowDiv = document.createElement("div");
    rowDiv.setAttribute("class","row");
    rowDiv.setAttribute("style","display:flex; background-color:#ffffff;");
    
    var rightbtHtml ='<div class="pull-right">' +
    '<div class="btn-group">' +
    	'<button type="button" class="btn btn-default dropdown-toggle pull-right" data-toggle="dropdown" style="border:none">' +
    	'<span class="caret"></span>' +
    	'</button>' +
    	'<span class="dropdown-menu" role="navigation">' +
    		'<p>' + data.description +
    		'</p>' +
    	'</span>' + 
    '</div>';
    rightbtHtml = '<div class="popover-options pull-right ">' +
    			  '<a href="#" type="button" class="btn btn-default" title="<h4>Title</h4>"  style="border:none"' +
    		      'data-container="body" data-placement="bottom" data-toggle="popover"' +
    		      'data-content="<h6>Popover 中的一些内容 —— options 方法</h6>">' +
    		      '<span class="caret" ></span>' +
    	          '</a>' +
                  '</div>';
    
    rightbtHtml = '<a data-toggle="collapse" data-parent="#accordion"' +
		   		' href="#' + descriptionId +'">' +
	...
	var rowDivDetail = document.createElement("div");
	rowDivDetail.setAttribute("class","row");
	rowDivDetail.setAttribute("style","display:flex; background-color:#ffffff;");
	var htmlDetail = '<div class="col-sm-12">' + data.description + '</div>';
	rowDivDetail.innerHTML = htmlDetail;
	rowDivDetail.setAttribute("id",descriptionId);
	
	var thisdiv = document.createElement("div");
	thisdiv.appendChild(rowDiv);
	thisdiv.appendChild(rowDivDetail);
	
	return thisdiv;
}

function addContentElement0(data,statusImg){
	var rowDiv = document.createElement("div");
    rowDiv.setAttribute("class","row");
    rowDiv.setAttribute("style","display:flex; background-color:#ffffff;");
